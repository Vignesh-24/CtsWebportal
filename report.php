...
class mypage extends FPDF
{
    ...
    function content()
    {
        ...
        $res=mysqli_query($scon,"select * from user_info where status=1 order by priority");
        $i=1;
        $width=190;
        $lineHeight=5;
        ...
        if($no==18)
        {
            $this->Cell(70,12,'Go To Cts Student Club Web portal', 0,1,'B',false, "https://ctsclubsjce.000webhostapp.com/index.php");
        }
        ...
        $this->AddPage();
            ...
            $this->Cell( 80, 50, $this->Image($image1, $this->GetX(), $this->GetY(), 60), 0, 0, 'L', false );
            if($image2=='')
            {
                $this->ln();
                $i++;
            ...
            $this->Cell( 80, 50, $this->Image($image2, $this->GetX(), $this->GetY(), 60), 0, 0, 'R', false );
            $this->ln();
...
$this->SetXY($x + 45, $y);
        ...
        $t11="UI Design";
        $t12="Technical Debate";
        $t13="Group Discussion";
        $width=190;
        $lineHeight=7;
        $this->setFont('Arial','',12);
       
        $this->MultiCell($width, $lineHeight, $t11);
          $this->MultiCell($width, $lineHeight, $t12);
            $this->MultiCell($width, $lineHeight, $t13);
        $this->setFont('Arial','B',12);
        $this->cell(0,10,'Suggestion or Feedbacks:',0,1,'L');
        $this->setFont('Arial','',12);
        $this->MultiCell($width, $lineHeight, "Nothing");
            $this->setFont('Arial','B',12);
